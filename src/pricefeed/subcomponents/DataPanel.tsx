    ...
    <div className="data">
      <div className="selection-bar">
        ...
        <div className="selections">
          {indices.map((n, i) => (
            <div
              key={n.name}
              className={"selection" + (selectedSynthIndex === i ? " selected" : "")}
            ...
            return (
              <div className="dropdown-option" key={trueIndex}>
                <div className="dropdown-option-inner">
                  ...
                  <Price currencyKey={n.currencyKey} classString="index-price" />
                  <span className="volume">{Number(oneDayVolumes[trueIndex]).toFixed(2)}</span>
                  <span className={"percent-change " + (Number(oneDayChanges[trueIndex]) > 0 ? "pos" : "neg")}>
      ...
      <div className="chart">
        ...
        <div className="index">
          <div className="title">
            <div
              className="icon"
              style={{
                backgroundImage: "url(" + indices[selectedSynthIndex].icon + ")",
              }}
            ></div>
            <div className="index-title">
              <span className="index-name">{indices[selectedSynthIndex].name}</span>
              <br />
          ...
          <div className="prices">
            ...
            <div className="price 24-vol">
              <span className="figure">{Number(oneDayVolumes[selectedSynthIndex]).toFixed(2)}</span>
              <br />
              24H Vol
            </div>
            <div className="price 24-change">
              <span className={"figure " + (Number(oneDayChanges[selectedSynthIndex]) > 0 ? "pos" : "neg")}>
        ...
        <div className="chart-tabs">
          <div
            className={"tab chart" + (activeTab === ChartTab.Chart ? " selected" : "")}
            onClick={() => setActiveTab(ChartTab.Chart)}
          >
            Chart
          </div>
          <div
            className={"tab details" + (activeTab === ChartTab.Details ? " selected" : "")}
        ...
        <div className="chart-content" onMouseLeave={() => setHoverPrice("")}>
          {activeTab === ChartTab.Chart ? (
            [
              ...
              <VictoryChart
                ...
                containerComponent={
                  <VictoryVoronoiContainer
                    ...
                    onActivated={(points) => setHoverPrice(points[0].y.toFixed(5))}
              ...
              >
                <VictoryAxis
                  ...
                  tickFormat={(t) => new Date(t).toLocaleDateString() + ", " + new Date(t).toLocaleTimeString()}
                />
                <VictoryAxis scale="linear" dependentAxis tickCount={10} />
                <VictoryLine
                  data={historicalPrices[selectedSynthIndex].map((p: PricePoint) => ({
                    x: p.timestamp,
            ...
            <div className="details">{indices[selectedSynthIndex].description}</div>
          )}
        </div>
      </div>
    </div>
